...
const getStock = async (req, res) => {
  try {
    const stock = await getStockId(req.params.id);
    res.status(200).json(stock);
  } catch (error) {
    res.status(500).send("getStock Error");
  }
};

const updateStockAtLocation = async (req, res) => {
  try {
    const loc = await updateLocationStock(
      req.params.id,
      req.body.masterAmount,
      req.body.innerAmount,
      req.body.unitAmount
    );
